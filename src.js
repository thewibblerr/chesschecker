...
function ensureEnhancements() {
  ...
  wrapper.innerHTML = `
    <div id="opening-panel" class="enhancement-block compact-block" hidden>
      <span class="eyebrow">Opening</span><div id="opening-name" class="opening-name"></div>
    </div>
    ...
    <div id="engine-panel" class="enhancement-block engine-panel">
      <div class="enhancement-heading"><div><span class="eyebrow">On-device analysis</span><h3>Best moves</h3></div><span id="engine-badge" class="mini-pill">Stockfish</span></div>
  ...
  [null,'K','Q','R','B','N','P','k','q','r','b','n','p'].forEach(piece => {
    const b = document.createElement('button'); b.type='button'; b.className='piece-option'; b.dataset.piece=piece||'';
    b.textContent = piece ? pieceGlyph[piece] : 'Empty'; b.onclick=()=>setSelectedSquare(piece); options.appendChild(b);
  });
...
function updateValidity(){
  ...
  const wk=board.filter(x=>x==='K').length,bk=board.filter(x=>x==='k').length;
  const ok=wk===1&&bk===1; pill.textContent=ok?'Looks valid':'Check kings'; pill.classList.toggle('warning',!ok);
}
...
function setSelectedSquare(piece){
  if(selectedSquare==null)return; board[selectedSquare]=piece||null; placement=boardToPlacement(board); bestMoveUci='';
  updateFen(); renderBoard(); updateOpening(); clearEngineResults('Position corrected. Ready to analyse.'); saveCurrentToHistory(); closePiecePicker();
}

function openingForFen(fen){ const [p,t]=fen.split(' '); return OPENINGS.get(`${p} ${t}`)||''; }
function updateOpening(){
  const fen=$('#fen')?.textContent||''; const name=openingForFen(fen); const panel=$('#opening-panel');
...
async function copyShareLink(){
  const url=shareUrl(); if(!url)return; const ok=await copyText(url); $('#copy-link').textContent=ok?'Copied ✓':'Copy failed'; setTimeout(()=>$('#copy-link').textContent='Copy link',1200);
}
...
function saveCurrentToHistory(extra={}){
  ...
  items.unshift({fen,ts:Date.now(),opening:openingForFen(fen),thumb:currentThumb||'',best:bestMoveUci||'',...extra}); putHistory(items); renderHistory();
}
function updateHistoryAnalysis(){
  ...
  const items=getHistory(); const hit=items.find(x=>x.fen===fen); if(hit&&rows[0]){hit.best=rows[0].move;hit.eval=normalisedEvaluation(rows[0]);hit.ts=Date.now();putHistory(items);renderHistory();}
}
function renderHistory(){
  const list=$('#history-list'); if(!list)return; const items=getHistory(); list.innerHTML='';
...
async function makeThumbnail(file){
  try{
    const img=await createImageBitmap(file); const max=160, scale=Math.min(1,max/img.width), c=document.createElement('canvas');
    c.width=Math.max(1,Math.round(img.width*scale)); c.height=Math.max(1,Math.round(img.height*scale)); c.getContext('2d').drawImage(img,0,0,c.width,c.height); img.close?.();
...
async function inferTurnFromScreenshot(file,corners,bottomSide){
  ...
  try{
    ...
    if(!top||!bottom)return fallback; const diff=(top.mean+top.bright*90)-(bottom.mean+bottom.bright*90); if(Math.abs(diff)<18)return fallback;
    const topSide=bottomSide==='w'?'b':'w'; return{side:diff>0?topSide:bottomSide,source:'active-clock'};
  }catch(_){return fallback;}
}

function legacyCopy(text){const ta=document.createElement('textarea');ta.value=text;ta.readOnly=true;ta.style.position='fixed';ta.style.left='-9999px';document.body.appendChild(ta);ta.select();let ok=false;try{ok=document.execCommand('copy');}catch(_){}ta.remove();return ok;}
async function copyText(text){if(legacyCopy(text))return true;try{await navigator.clipboard.writeText(text);return true;}catch(_){return false;}}
async function tryAutomaticCopy(fen){try{await navigator.clipboard.writeText(fen);return true;}catch(_){return false;}}

async function scan(file){
  ensureEnhancements(); $('#status').textContent='Reading chessboard…'; $('#result').hidden=true; $('#preview').src=URL.createObjectURL(file); $('#preview').hidden=false;
  try{
    ...
    const turn=await inferTurnFromScreenshot(file,result.corners,boardFlipped?'b':'w'); $('#turn').value=turn.side;
    ...
    $('#confidence').textContent=Math.round(result.meanConfidence*100)+'%'; currentThumb=await makeThumbnail(file); const fen=updateFen(); renderBoard(); clearEngineResults('Analyse the recognised position directly on this device.'); $('#result').hidden=false; saveCurrentToHistory();
    const copied=await tryAutomaticCopy(fen),recognised=result.reliable?'Board recognised.':'Board recognised with low confidence. Check the board and correct any piece if needed.';
...
async function scanClipboardImage(){
  ...
  try{const items=await navigator.clipboard.read();for(const item of items){const type=item.types.find(t=>t.startsWith('image/'));if(!type)continue;const blob=await item.getType(type);await scan(new File([blob],'clipboard-screenshot',{type}));return;}$('#status').textContent='No image found on the clipboard.';}
...
function waitForEngine(worker,command,expected,timeout=12000){return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{worker.removeEventListener('message',handler);reject(new Error('Stockfish timed out'));},timeout);const handler=e=>{if(String(e.data||'').includes(expected)){clearTimeout(timer);worker.removeEventListener('message',handler);resolve();}};worker.addEventListener('message',handler);worker.postMessage(command);});}
async function getEngine(){
  if(engineWorker)return engineWorker;if(engineLoading)return engineLoading;
  engineLoading=(async()=>{ $('#engine-status').textContent='Loading Stockfish on this device…'; const response=await fetch('https://cdn.jsdelivr.net/npm/stockfish@18.0.8/bin/stockfish-18-asm.js'); if(!response.ok)throw new Error('Could not load Stockfish');
    const blobUrl=URL.createObjectURL(new Blob([await response.text()],{type:'text/javascript'})),worker=new Worker(blobUrl);worker.addEventListener('message',handleEngineMessage);await waitForEngine(worker,'uci','uciok');worker.postMessage('setoption name MultiPV value 3');await waitForEngine(worker,'isready','readyok');engineWorker=worker;return worker;})().finally(()=>engineLoading=null);return engineLoading;
}
function parseEngineInfo(line){const multipv=Number(line.match(/\bmultipv\s+(\d+)/)?.[1]||1),depth=Number(line.match(/\bdepth\s+(\d+)/)?.[1]||0),score=line.match(/\bscore\s+(cp|mate)\s+(-?\d+)/),pv=line.match(/\bpv\s+([a-h][1-8][a-h][1-8][qrbn]?)/);return score&&pv?{multipv,depth,type:score[1],value:Number(score[2]),move:pv[1]}:null;}
function normalisedEvaluation(info){let value=info.value;if($('#turn').value==='b')value=-value;if(info.type==='mate')return value>0?`Mate +${value}`:`Mate ${value}`;const pawns=value/100;return(pawns>0?'+':'')+pawns.toFixed(2);}
function moveToSan(uci){try{const chess=new Chess($('#fen').textContent);return chess.move({from:uci.slice(0,2),to:uci.slice(2,4),promotion:uci[4]||'q'})?.san||uci;}catch(_){return uci;}}
...
function handleEngineMessage(e){
  const line=String(e.data||'');if(line.startsWith('info ')){const info=parseEngineInfo(line);if(info){const previous=engineLines.get(info.multipv);if(!previous||info.depth>=previous.depth)engineLines.set(info.multipv,info);renderEngineResults();$('#engine-status').textContent=`Analysing… depth ${info.depth}`;}}
  if(line.startsWith('bestmove')){engineBusy=false;const best=line.match(/^bestmove\s+([^\s]+)/)?.[1];if(best&&best!=='(none)')bestMoveUci=best;renderEngineResults();$('#engine-status').textContent=engineLines.size?'Best move highlighted on the board. Evaluation is from White’s point of view.':'Analysis finished.';$('#find-best').textContent='Analyse again';updateHistoryAnalysis();}
}
async function analyseWithStockfish(){
  const fen=$('#fen').textContent;if(!fen||engineBusy)return;try{new Chess(fen);}catch(_){$('#engine-status').textContent='Correct the position before analysing. It is not a valid chess position.';return;}
  const btn=$('#find-best');btn.textContent='Analysing…';engineLines.clear();bestMoveUci='';renderBoard();$('#engine-results').innerHTML='';
  try{const worker=await getEngine();engineBusy=true;worker.postMessage('stop');worker.postMessage('ucinewgame');worker.postMessage(`position fen ${fen}`);worker.postMessage('go depth 14');}
  catch(e){console.error(e);engineBusy=false;btn.textContent='Find best move';$('#engine-status').textContent='Stockfish could not load. You can still analyse on Lichess or Coachess.';}
}

function loadFen(fen,fromUrl=false,thumb=''){
  ensureEnhancements();
  try{new Chess(fen);}catch(_){if(fromUrl)$('#status').textContent='That shared position is invalid.';return false;}
  const parts=fen.split(' ');placement=parts[0];board=parsePlacement(placement);$('#turn').value=parts[1]==='b'?'b':'w';boardFlipped=new URLSearchParams(location.search).get('pov')==='black';currentThumb=thumb||'';bestMoveUci='';
  $('#confidence').textContent='Shared';$('#preview').hidden=true;updateFen();renderBoard();clearEngineResults('Analyse this position directly on this device.');$('#result').hidden=false;$('#status').textContent=fromUrl?'Shared position loaded.':'Recent position loaded.';saveCurrentToHistory();return true;
}
function loadFenFromUrl(){const fen=new URLSearchParams(location.search).get('fen');if(fen)loadFen(fen,true);}
...
window.addEventListener('paste',e=>{const item=[...(e.clipboardData?.items||[])].find(i=>i.type.startsWith('image/'));const f=item?.getAsFile();if(f)scan(f);});
loadFenFromUrl();
